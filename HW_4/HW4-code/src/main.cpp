...
bool testShortestPath() {
    ...
    std::cout << "Path from 0 to 5 pass by 3: ";
    std::vector<int> path = G.search(0, 3, 5);
    int startIndex = 0;
    int destIndex = 5;
    int passByIndex = 3;
    if (path.size() < 3) {
        std::cout << "Your implementation is incorrect! The number of vertices on the path should not be less than 3" << std::endl;
        exit(-1);
    } else if (path.front() != startIndex) {
        std::cout << "Your implementation is incorrect! The starting vertex on your path is incorrect" << std::endl;
        exit(-1);
    } else if (path.back() != destIndex){
        std::cout << "Your implementation is incorrect! The destination vertex on your path is incorrect" << std::endl;
        exit(-1);
    } else if (std::find(path.begin(), path.end(), passByIndex) == path.end()) {
        std::cout << "Your implementation is incorrect! The pass by vertex is not on your path" << std::endl;
        exit(-1);
    }
...
void searchOnCampus(std::string start = "BELL", std::string passBy = "JBHT", std::string destination = "HAPG") {
    std::ifstream reader("assets/map_info.txt");
    int n, m;
    reader >> n >> m;
    std::map<std::string, int> name2index;
    ...
    std::vector<int> path = G.search(name2index[start], name2index[passBy], name2index[destination]);

    int startIndex = name2index[start];
    int destIndex = name2index[destination];
    int passByIndex = name2index[passBy];
    if (path.size() < 3) {
        std::cout << "Your implementation is incorrect! The number of vertices on the path should not be less than 3" << std::endl;
        exit(-1);
    } else if (path.front() != startIndex) {
    ...
    std::cout << "Path from " << start  << " to " << destination << " Pass By " << passBy << ": " << start;
